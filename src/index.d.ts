...
import { RequestOptions as TRequestOptions, Response, fetch } from '@tauri-apps/api/http';
...
export interface ClientOptions extends Omit<TRequestOptions, 'headers' | 'method'> {
  /** set the common root URL for all API requests */
  baseUrl?: string;
  /** custom fetch (defaults to fetch) */
  fetch?: typeof fetch;
...
type BodyType<T = unknown> = {
  json: T;
  //text: Awaited<ReturnType<Response<T>['text']>>;
  //blob: Awaited<ReturnType<Response<T>['blob']>>;
  //arrayBuffer: Awaited<ReturnType<Response<T>['arrayBuffer']>>;
  //stream: Response<T>['body'];
};
...
export type FetchOptions<T> = RequestOptions<T> & Omit<TRequestOptions, 'body'>;

export type FetchResponse<T, O> =
  | {
      data: ParseAsResponse<FilterKeys<SuccessResponse<ResponseObjectMap<T>>, MediaType>, O>;
      error?: never;
  ...
  | {
      ...
      response: Response<FilterKeys<ErrorResponse<ResponseObjectMap<T>>, MediaType>>;
    };
...
export interface MiddlewareRequest extends TRequestOptions {
...
): TRequestOptions | undefined | Promise<TRequestOptions | undefined>;
export function onResponse(
  res: Response<unknown>,
  options: MergedOptions
): Response<unknown> | undefined | Promise<Response<unknown> | undefined>;
...
export type ClientMethod<Paths extends Record<string, PathMethods>, M extends HttpMethod> = <
